...
func strace(c *cli.Context) error {
	...
	for {
		...
		if err != nil {
			return fmt.Errorf("error while instructing child pid %d to run until the next syscall: %w", childPid, err)
		}

		// Wait for the child to send us a trap when it does syscalls
		waitStatus, err := waitPid(childPid)
		if err != nil {
			return fmt.Errorf("error while waiting for child pid %d: %w", childPid, err)
		}
		...
		if traps > 0 && traps%2 == 0 {
			...
			if err = syscall.PtraceGetRegs(childPid, &regs); err != nil {
				return fmt.Errorf("error while reading Orig_rax for child pid %d: %w ", childPid, err)
			}

			if err = printSyscall(childPid, regs); err != nil {
				return fmt.Errorf("error while printing syscall for child pid %d: %w", childPid, err)
			}
...
// Prints syscall info in a manner similar to strace
...
func readOrigRax(pid int) (uint64, error) {
	var regs syscall.PtraceRegs
	return regs.Orig_rax, syscall.PtraceGetRegs(pid, &regs)
}

// taken from https://github.com/strace/strace/blob/v4.26/linux/x86_64/syscallent.h
var syscallNameByRax = map[uint64]string{
	0:   "read",
	1:   "write",
	2:   "open",
	3:   "close",
